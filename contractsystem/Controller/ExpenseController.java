...
public class ExpenseController {
    ...
    @PostMapping("/add")
    public ResponseEntity addExpense(@RequestBody @Valid Expense expense, Errors errors) {
        if (errors.hasErrors()) {
    ...
    public ResponseEntity updateExpense(@PathVariable Integer id,@RequestBody @Valid Expense expense, Errors errors) {
        if (errors.hasErrors()) {
            return ResponseEntity.status(400).body(errors.getFieldError().getDefaultMessage());
        }
