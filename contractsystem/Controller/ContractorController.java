...
public class ContractorController {
    ...
    @PostMapping("/add")
    public ResponseEntity addContractor(@RequestBody @Valid Contractor contractor, Errors errors) {
        if (errors.hasErrors()) {
    ...
    public ResponseEntity updateContractor(@PathVariable Integer id ,@RequestBody @Valid Contractor contractor, Errors errors) {
        if (errors.hasErrors()) {
            return ResponseEntity.status(400).body(errors.getFieldError().getDefaultMessage());
        }
