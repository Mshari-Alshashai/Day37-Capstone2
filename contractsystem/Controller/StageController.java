...
public class StageController {
    ...
    @PostMapping("/add")
    public ResponseEntity addStage(@RequestBody @Valid Stage stage, Errors errors) {
        if (errors.hasErrors()) {
    ...
    public ResponseEntity updateStage(@PathVariable Integer id, @RequestBody @Valid Stage stage, Errors errors) {
        if (errors.hasErrors()) {
            return ResponseEntity.status(400).body(errors.getFieldError().getDefaultMessage());
        }
